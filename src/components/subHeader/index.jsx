...
function SubHeader(props){
    ...
    async function handleRouteUpdate(pos){
        await handleRoute(pos);
        props.history.push(routes[route]);
    }
    const svg = (
        ...
        <SubHeaderStyle>
            <div className="btn-container">
                <div className="btn-previous"> 
                    {svg}  
                    <Button type="btn" fn={() => handleRouteUpdate(route < 1 ? 0 : route - 1)}>
                        Previous
                    </Button>
                </div>
                <div className="btn-next">
                    <Button type="btn" fn={() => handleRouteUpdate(route === routes.length-1 ? routes.length-1 : route + 1)}> 
                        Next
                    </Button>
                    {svg}
                </div>
